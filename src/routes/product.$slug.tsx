...
import { Star, Heart, Truck, ShieldCheck, RotateCcw, Minus, Plus, ShoppingCart, ChevronRight, Maximize2, Timer, MessageSquare, Send, Loader2, X, Share2, Facebook, Link as LinkIcon } from "lucide-react";
      ...
      <div className="mt-5 grid lg:grid-cols-[1fr_1fr] gap-8 lg:gap-12">
        ...
        <div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5"><Share2 className="size-3" /> Share</span>
            <button 
              onClick={() => {
                const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`;
                window.open(url, "_blank");
              }}
              className="size-8 rounded-full bg-blue-600 text-white grid place-items-center hover:bg-blue-700 transition-colors"
            ...
            <button 
              onClick={() => {
                const url = `https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out ${p.name} at buyphone.lk! ${window.location.href}`)}`;
                window.open(url, "_blank");
              }}
              className="size-8 rounded-full bg-green-500 text-white grid place-items-center hover:bg-green-600 transition-colors"
            ...
            <button 
              ...
              className="size-8 rounded-full bg-muted text-muted-foreground grid place-items-center hover:bg-muted/80 transition-colors"
